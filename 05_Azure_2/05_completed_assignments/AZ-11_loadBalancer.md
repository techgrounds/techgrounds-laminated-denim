# Azure Load Balancer
Azure Load Balancer is an Azure resource that functions as the on-premises equivalent of a load balancer. There are a variety of resources that can fill this function, each with different features. Load Balancer, Azure Application Gateway and Front Door are three examples.

#Azure/Services #Azure/AZ-900 #Azure/Management 
...
### Resultaat
To start, I created a Virtual Machine Scale Set with a load balancer and the settings defined by the assignment.  
![ss2](../../00_includes/AZ-11_screenshot2.png)  
I checked the Load Balancer's public IP and noted the NAT rules to find the correct port to connect to each VM via SSH.  
![ss3](../../00_includes/AZ-11_screenshot3.png)  
Following this, I downloaded and ran the stress tool using the command `sudo stress --cpu 8 --timeout 600 &` . This allowed me to bring the CPU on both machines to 100% utilisation.  
![ss4](../../00_includes/AZ-11_screenshot4.png)  
![ss5](../../00_includes/AZ-11_screenshot5.png)  
After a few minutes at 100% CPU utilisation, the autoscaler kicked in and created a new VM instance.
